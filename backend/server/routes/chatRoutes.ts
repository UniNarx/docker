...
import { Router, Response } from 'express'; // Убедимся, что Response импортирован из express
...
// Функция для генерации ID диалога
const generateConversationId = (userId1: string, userId2: string): string => {
  return [userId1, userId2].sort().join('_');
};

// GET /api/chat/conversations - Получить список диалогов для текущего пользователя
router.get('/conversations', protect, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  ...
  if (!currentUserId) {
    ...
    return; // Явный return для выхода и соответствия Promise<void>
  }
...
// GET /api/chat/history/:otherUserId - Получить историю сообщений с конкретным пользователем
...
// POST /api/chat/messages/:messageId/read - Пометить сообщение как прочитанное
